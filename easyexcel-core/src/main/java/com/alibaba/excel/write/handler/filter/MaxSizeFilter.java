...
public class MaxSizeFilter extends BasePipeFilter<Object, Object> {
    ...
    public PipeDataWrapper<Object> apply(PipeDataWrapper<Object> wrapper) {
        ...
        try {
            ...
            if (!StringUtils.isNumeric(param)) {
                ...
                if (param.endsWith(KB_UPPER) || param.endsWith(KB_LOWER)) {
                    ...
                    if (!StringUtils.isNumeric(param)) {
                        return PipeDataWrapper.error(errorPrefix() + "参数去掉[KB/kb]后应该是数字");
                    }
                    paramInt = Integer.parseInt(param) * 1000;
                } else if (param.endsWith(MB_UPPER) || param.endsWith(MB_LOWER)) {
                    param = param.substring(0, param.length()-2);

                    if (!StringUtils.isNumeric(param)) {
                        return PipeDataWrapper.error(errorPrefix() + "参数去掉[MB/mb]后应该是数字");
                    }
                    paramInt = Integer.parseInt(param) * 1000 * 1000;
                } else if (param.endsWith(GB_UPPER) || param.endsWith(GB_LOWER)) {
                    param = param.substring(0, param.length()-2);
                    if (!StringUtils.isNumeric(param)) {
                        return PipeDataWrapper.error(errorPrefix() + "参数去掉[GB/gb]后应该是数字");
                    }
                    paramInt = Integer.parseInt(param) * 1000 * 1000 * 1000;
                } else {
            ...
            if (value instanceof String) {
                ...
                if (!StringUtils.isNumeric(val)) {
                    if (val.endsWith(KB_UPPER) || val.endsWith(KB_LOWER)) {
                        ...
                        valueInt = Integer.parseInt(val) * 1000;
                        unit = KB_UPPER;
                        ...
                        valueInt = Integer.parseInt(val) * 1000 * 1000;
                        unit = MB_UPPER;
                        ...
                        valueInt = Integer.parseInt(val) * 1000 * 1000 * 1000;
                        unit = GB_UPPER;
                    } else {
                        return PipeDataWrapper.error(errorPrefix() + "传入数据类型应该是数字");
                    }
                ...
                valueInt = ((Number) value).intValue() * 1000;
            } else {
                return PipeDataWrapper.error(errorPrefix() + "传入数据类型应该是数字");
            }
        ...
        switch (unit) {
            case MB_UPPER:
                valueResult = (valueInt / 1000 / 1000) + KB_UPPER;
                break;
            case GB_UPPER:
                valueResult = (valueInt / 1000 / 1000 / 1000) + KB_UPPER;
                break;
            default:
                valueResult = (valueInt / 1000) + KB_UPPER;
        }
        return PipeDataWrapper.success(valueResult);
    }
}
